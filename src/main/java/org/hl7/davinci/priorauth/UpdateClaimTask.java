...
class UpdateClaimTask extends TimerTask {
    ...
    public void run() {
        ...
        if(updateDiffRequest(bundle, claimId, patient) != null) {
            // Check for subscription
            Map<String, Object> constraintMap = new HashMap<String, Object>();
            constraintMap.put("claimResponseId", claimId);
            constraintMap.put("patient", patient);
    ...
    /**
     * Update the claim, that the differential request references and generate a new ClaimResponse.
     *
     * @param bundle      - the Bundle the Claim is a part of.
     * @param claimId     - the Claim ID.
     * @param patient     - the Patient ID.
     */
    public Bundle updateDiffRequest(Bundle bundle, String claimId,  String patient){

        logger.info("ClaimEndpoint::updateClaim(" + claimId + "/" + patient + ")");
        // Get the full not the smaller version claim from the database

        Claim claim = (Claim) App.getDB().read(Table.CLAIM, Collections.singletonMap("id", claimId));

        if (claim != null && !FhirUtils.isCancelled(Table.CLAIM, claimId))
            return ClaimResponseFactory.generateAndStoreClaimResponse(bundle, claim, claimId, Disposition.UNKNOWN,
                    ClaimResponseStatus.ACTIVE, patient);
